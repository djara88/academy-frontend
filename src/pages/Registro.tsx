...
const Registro: React.FC = () => {
  ...
  const handleRegistroManual = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Llamamos a la API de tu backend para que haga todo el trabajo pesado
      // (Crear usuario en Auth, crear academia en DB, vincular y enviar correo)
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0d1117] p-4 font-sans">
      <div className="bg-[#161b22] p-8 rounded-xl border border-[#30363d] shadow-2xl w-full max-w-md">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white hover:bg-gray-100 text-gray-900 font-bold py-2.5 px-4 rounded-md transition-colors flex items-center justify-center gap-3 mb-6"
        >
        ...
        <form onSubmit={handleRegistroManual} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-[#238636] hover:bg-[#2ea043] text-white font-bold py-3 px-4 rounded-md transition-colors mt-2">
